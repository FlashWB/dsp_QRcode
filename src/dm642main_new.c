...
float getAngle1()
{
	...
	int x1,y1,x2,y2;
	int dot1,dot2;
	int lastdot_ji,nowdot_ji;
	int lastdot_ou,nowdot_ou;
	lastdot_ji = tempYbuffer + i*numPixels + j;
	nowdot_ji = tempYbuffer + i*numPixels + j+1;

	lastdot_ou = tempYbuffer + (numLines/2+i)*numPixels + j;
	nowdot_ou = tempYbuffer + (numLines/2+i)*numPixels + j + 1;

	for (i = intALines;i < intDLines; i++)
	{
		for (j = intAPixels; j < intDPixels - 1; j++)
		{
			if (lastdot_ji==0x00 &&nowdot_ji==0xFF)
			{
	...
	return atan2(abs(y1-y2),abs(x1-x2));
}
	...
	for(i=numLines/2+intALines;i<numLines/2+intDLines;i++)
	{
	    ...
	    {		
            ...
            { 
			    //传送亮度信号
			    //*(Uint8 *)(tempDisYbuffer + i*numPixels + j) = *(Uint8 *)(capYbuffer + intCapY*numPixels + intCapX); 		               	           	           	           	
			    *(Uint8 *)(tempDisYbuffer + i*numPixels + j) = *(Uint8 *)(tempThrebuffer + intCapY*numPixels + intCapX); 		               	           	           	           	
        	} 
            else
            {
            	 *(Uint8 *)(tempDisYbuffer + i*numPixels + j) = 0xFF;
